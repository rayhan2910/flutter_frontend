...
class TypeDataSource extends BsDatatableSource {
  ...
  static List<BsDataColumn> columns = <BsDataColumn>[
    BsDataColumn(
        ...
        orderable: false),
    BsDataColumn(
        label: Text(TypeText.formCode), columnName: TypeText.tableCode),
    BsDataColumn(label: Text(DBText.formName), columnName: TypeText.tableName),
    BsDataColumn(
        label: Text(TypeText.formParent), columnName: TypeText.tableParentId),
    BsDataColumn(
  ...
  List<TypeModel> get types => List<TypeModel>.from(
      response.data.map((e) => TypeModel.fromJson(e)).toList());
